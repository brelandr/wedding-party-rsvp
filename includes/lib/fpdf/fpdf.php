...
// phpcs:disable WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedClassFound -- Third-party FPDF; class name is public API.
...
function Output($dest='', $name='', $isUTF8=false)
{
	...
	switch(strtoupper($dest))
	{
		case 'I':
			...
			// phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Raw binary PDF body for inline display.
			echo $this->buffer;
			break;
		case 'D':
			// Download file
			...
			header('Cache-Control: private, max-age=0, must-revalidate');
			header('Pragma: public');
			// phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Raw binary PDF body for download.
			echo $this->buffer;
			break;
		case 'F':
			// Save to local file
			// phpcs:ignore WordPress.WP.AlternativeFunctions.file_system_operations_file_put_contents -- FPDF writes binary PDF to caller-supplied path; WP_Filesystem not used by upstream.
...
protected function _parsejpg($file)
{
	...
	// phpcs:ignore WordPress.WP.AlternativeFunctions.file_get_contents_file_get_contents -- Local binary JPEG payload for embedding (not a remote URL).
...
protected function _parsepng($file)
{
	...
	// phpcs:ignore WordPress.WP.AlternativeFunctions.file_system_operations_fopen -- FPDF stream read; not replaceable by WP_Filesystem for binary chunk parsing.
	$f = fopen($file,'rb');
	if(!$f)
...
protected function _readstream($f, $n)
{
	...
	while($n>0 && !feof($f))
	{
		// phpcs:ignore WordPress.WP.AlternativeFunctions.file_system_operations_fread -- Binary PNG stream read.
		$s = fread($f,$n);
		if($s===false)
...
protected function _parsegif($file)
{
	...
	$data = ob_get_clean();
	imagedestroy($im);
	// phpcs:ignore WordPress.WP.AlternativeFunctions.file_system_operations_fopen -- In-memory temp stream for GIF→PNG pipeline.
	$f = fopen('php://temp','rb+');
	if(!$f)
...
protected function _putfonts()
{
	foreach($this->FontFiles as $file=>$info)
	{
		...
		// phpcs:ignore WordPress.WP.AlternativeFunctions.file_get_contents_file_get_contents -- Local binary font payload for PDF embedding.
...
{
	$date = gmdate( 'YmdHisO', $this->CreationDate );
