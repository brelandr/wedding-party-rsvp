...
if ( ! class_exists( 'WGRSVP_Privacy' ) ) {
	...
	class WGRSVP_Privacy {
		...
		public static function register_privacy_policy_content() {
			...
			$suggested .= '<p class="privacy-policy-tutorial">' . esc_html__( 'The optional public guestbook stores visitor names and messages you approve for display (database table typically wp_wgrsvp_guestbook). Guestbook rows are not matched to guest email addresses; erase guest email data does not automatically remove guestbook notes signed under a different name.', 'wedding-party-rsvp' ) . '</p>';
			$suggested .= '<p class="privacy-policy-tutorial">' . esc_html__( 'If the site owner configures Google reCAPTCHA and/or Cloudflare Turnstile for the guestbook, the guest’s browser loads those providers’ scripts and a verification token (and typically the visitor IP) is sent to Google or Cloudflare when a note is submitted. Keys are stored only on this WordPress site; enable only providers you have documented in your own privacy policy.', 'wedding-party-rsvp' ) . '</p>';
			$suggested .= '</div>';
