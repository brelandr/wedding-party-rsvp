...
if ( ! class_exists( 'WGRSVP_Wedding_RSVP' ) ) :
	...
	class WGRSVP_Wedding_RSVP {
		...
		public function enqueue_admin_styles() {
			wp_register_style( 'wgrsvp-admin-style', false, array(), '8.0.0' );
		...
		public function enqueue_frontend_styles() {
			wp_register_style( 'wgrsvp-front-style', false, array(), '8.0.0' );
		...
		public function admin_page_guests() {
			...
			// Handle CSV Import
			if ( isset( $_POST['wgrsvp_import_guests_nonce'] ) && wp_verify_nonce( wp_unslash( $_POST['wgrsvp_import_guests_nonce'] ), 'wgrsvp_import_guests' ) ) {
		...
		public function admin_page_settings() {
			// Use $_POST with wp_unslash() before wp_verify_nonce() per WordPress Coding Standards.
			$reset_nonce = isset( $_POST['wgrsvp_factory_reset_plugin_nonce'] ) ? wp_unslash( $_POST['wgrsvp_factory_reset_plugin_nonce'] ) : '';
			if ( $reset_nonce && wp_verify_nonce( $reset_nonce, 'wgrsvp_factory_reset_plugin' ) ) {
			...
			// 2. Save Settings
			$settings_nonce = isset( $_POST['wgrsvp_save_general_settings_nonce'] ) ? wp_unslash( $_POST['wgrsvp_save_general_settings_nonce'] ) : '';
			if ( $settings_nonce && wp_verify_nonce( $settings_nonce, 'wgrsvp_save_general_settings' ) ) {
		...
		public function admin_page_menu() {
			...
			$menu_nonce = isset( $_POST['wgrsvp_save_entree_menu_options_nonce'] ) ? wp_unslash( $_POST['wgrsvp_save_entree_menu_options_nonce'] ) : '';
			if ( $menu_nonce && wp_verify_nonce( $menu_nonce, 'wgrsvp_save_entree_menu_options' ) ) {
		...
		private function handle_admin_actions() {
			...
			$add_nonce = isset( $_POST['wgrsvp_add_guest'] ) ? wp_unslash( $_POST['wgrsvp_add_guest'] ) : '';
			...
			$edit_nonce = isset( $_POST['wgrsvp_edit_guest'] ) ? wp_unslash( $_POST['wgrsvp_edit_guest'] ) : '';
		...
		public function handle_csv_export() {
			// SECURITY FIX: Use filter_input with sanitized flag
			$export_nonce = isset( $_POST['wgrsvp_export_guest_list_nonce'] ) ? wp_unslash( $_POST['wgrsvp_export_guest_list_nonce'] ) : '';
			if ( $export_nonce && wp_verify_nonce( $export_nonce, 'wgrsvp_export_guest_list' ) ) {
		...
		public function process_frontend_submissions() {
			...
			if ( isset( $_POST['wpr_submit_rsvp'], $_POST['_wpnonce'] ) && wp_verify_nonce( wp_unslash( $_POST['_wpnonce'] ), 'wgrsvp_front_rsvp_submit' ) ) {
					global $wpdb;
		...
		public function render_frontend_form() {
			...
			if ( isset( $_POST['wgrsvp_front_party_login_submit'] ) ) {
				if ( ! isset( $_POST['wgrsvp_front_party_login_nonce'] ) || ! wp_verify_nonce( wp_unslash( $_POST['wgrsvp_front_party_login_nonce'] ), 'wgrsvp_front_party_login' ) ) {
